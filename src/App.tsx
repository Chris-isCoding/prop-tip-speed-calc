...
import tipSpeedCalculator from './the-math/the-math';
import { round } from './the-math/utilities';
...
export default function App() {
  ...
  const handlePropDia = (propDiameter: number) => {
    ...
    if (state.units === 'imperial') {
      ...
      propDiaMm = round(propDiameter * 25.4, 0);
    }
    if (state.units === 'metric') {
      propDiaIn = round(propDiameter / 25.4, 1);
  ...
  const calculate = () => {
    if (state.propDiaIn === 0 || state.propDiaMm === 0)
    ...
    const [MPH, KPH] = tipSpeedCalculator(inches, volts, killavolts, airspeed);

    const displayVal = state.units === 'imperial' ? MPH : KPH;

    const localTemp_c = state.altitude
      ? conditions.temp_c - state.altitude / 500
      : conditions.temp_c;
    const localMach1Km = parseInt(
      (((331.3 + 0.6 * localTemp_c) / 1000) * 3600).toFixed(1)
    );
